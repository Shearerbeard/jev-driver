...
fn schema_artifact_matches_derive() -> Result<(), Box<dyn std::error::Error>> {
    ...
    artifact.validate()?;
    assert_eq!(artifact, TicketTriage::schema(), "schema artifact drifted from the derive");
    Ok(())
}
...
fn request_body_is_golden() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(actual.trim(), REQUEST.trim(), "request wire JSON must match the golden fixture byte-for-byte");
    Ok(())
}
...
fn fixture_parses_into_typed_answers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(typed.department.selected, Department::Billing, "selected option round-trips into the enum");
    let p_technical = typed.department.probabilities.get(&Department::Technical).get();
    assert!((p_technical - 0.12).abs() < 1e-9, "per-variant probability is typed and indexed, got {p_technical}");
    assert!(typed.department.confidence.act(0.80), "confidence 0.81 clears the 0.80 act gate");
    assert!(!typed.department.confidence.act(0.99), "confidence 0.81 must not clear the 0.99 gate");

    assert_eq!(typed.frustration.nearest(), Frustration::Concerned, "score nearest level maps back to the rubric enum");
    assert!((typed.frustration.score - 1.05).abs() < 1e-9, "probability-weighted score round-trips, got {}", typed.frustration.score);

    assert!(typed.is_urgent.probability.yes(0.90), "noul 0.95 clears the 0.90 yes gate");
    Ok(())
}
...
fn dynamic_and_typed_layers_agree() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(dyn_department.selected, "billing", "dynamic layer sees the raw option key");
    let typed_key = typed.department.selected.option_name();
    assert_eq!(dyn_department.selected, typed_key, "typed enum maps to the same wire key");

    let owner = answers.choice("owner")?;
    assert_eq!(owner.selected, "dana", "dynamic DynChoice answer validates against its supplied option set");
    Ok(())
}
...
fn unknown_option_is_strictly_rejected() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let err = Answers::from_wire(mutated, &decision).expect_err("an option outside the supplied set must be rejected");
    match err {
...
fn unknown_answer_id_is_rejected() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let err = Answers::from_wire(mutated, &decision).expect_err("an answer for an unasked question must be rejected");
    match err {
        JevError::UnknownQuestion { id } => assert_eq!(id, "ghost", "error names the ghost question id"),
...
fn runtime_criteria_must_be_supplied() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match err {
        JevError::MissingCriteria { id } => assert_eq!(id, "department", "error names the unfilled question"),
...
fn state_is_required() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(err, JevError::MissingState), "expected MissingState, got {err:?}");
    Ok(())
}
...
fn typed_state_pairing_rejects_wrong_shapes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let decision = build_demo_decision()?;
    assert_eq!(
        decision.state()["account_tier"], json!("pro"),
...
fn dyn_choice_builder_enforces_cardinality() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(empty, Err(JevError::Schema(_))), "zero options must be a schema error, got {empty:?}");
    ...
    assert!(matches!(too_many, Err(JevError::Schema(_))), "256 options must exceed the 255 cardinality cap");
    Ok(())
}
...
fn schema_export_round_trips_through_json() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(reparsed, schema, "DecisionSchema IR must round-trip losslessly through JSON");
    assert_eq!(json_str.trim(), SCHEMA.trim(), "exported artifact matches the committed fixture");
    Ok(())
}
